...
def main():
    ...
    all_keys = sorted(set(count1.keys()).union(
        set(count2.keys())).union(set(count3.keys())))
    ...
    cud_colors = ['#E69F00', '#56B4E9', '#009E73',
                  '#F0E442', '#0072B2', '#D55E00', '#CC79A7']

    plt.bar(index, frequencies1, bar_width, alpha=0.6,
            color=cud_colors[1], label=f'GrammarFuzzer in {t_r1:.2f}s')
    plt.bar(index + bar_width, frequencies2, bar_width, alpha=0.6,
            color=cud_colors[0], label=f'ExGrammarFuzzer in {t_r2:.2f}s')
    plt.bar(index + 2 * bar_width, frequencies3, bar_width, alpha=0.6,
            color=cud_colors[2], label=f'ExProbablisticFuzzer in {t_r3:.2f}s')

    plt.xlabel('Input Length', fontsize=10)
    plt.ylabel('Frequency', fontsize=10)
    plt.title('Distribution of Input Length (for ' +
              str(X) + ' inputs)', fontsize=10)

    plt.yticks(fontsize=10)

    # Or, show only every nth label on the x-axis
    n = 5  # Show every 5th label
    plt.xticks(index[::n] + bar_width / 2,
               all_keys[::n], rotation=-45, fontsize=13)
